...
public class Main {
    ...
    public static void makeRole() {
        // now im making first n//5 mafias then n//5 detectives then n//10 healers and
        // remaining commoners
    ...
    public static void printDetails() {
        for (Player player : TotalPlayers) {
            System.out.println(
                    player.getPlayerID() + "\t" + player.getHP() + "\t" + player.getClass() + "\t" + player.isStatus());
        }
    }
            ...
            if (player.isStatus() && player.getPlayerID() != user.getPlayerID()) {
                ...
                if (user.getClass() != Commoner.class) {
                    while (!TotalPlayers[votingResult].isStatus()
                            || TotalPlayers[votingResult].getClass() == player.getClass()) {
                ...
                voteData.put(TotalPlayers[votingResult].getPlayerID(),
                        voteData.getOrDefault(TotalPlayers[votingResult].getPlayerID(), 0) + 1);
            }
        }
        voteData.put(TotalPlayers[vote - 1].getPlayerID(),
                voteData.getOrDefault(TotalPlayers[vote - 1].getPlayerID(), 0) + 1);
    ...
    private static void heal(int healedPlayer) {
        ...
        if (healedPlayer != -1)
            TotalPlayers[healedPlayer - 1].setHP(TotalPlayers[healedPlayer - 1].getHP() + 500);// healed the player
                                                                                               // chosen by healers
    }

    private static void detectiveTest(int testPlayer) { // return true if mafia is caught otherwise false
        if (TotalPlayers[testPlayer - 1].getClass() != Mafia.class) { // if detective test is negative i.e. Mafia is not
                                                                      // caught by detective
        ...
        if (user.isStatus()) {// if user is alive then input is taken
            ...
            while (true) {
                ...
                if (healedPlayer < 0 || healedPlayer > total_players) {
                    System.out.println("Invalid input!!, try again");
                    continue;
                } else if (targetPlayer != healedPlayer && !TotalPlayers[targetPlayer - 1].isStatus()) { // if test
                                                                                                         // player is
                                                                                                         // dead
                    System.out.println("Can't test a dead person");
                    continue;
